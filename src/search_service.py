...
if _tenacity_available:
    ...
    # 当 tenacity 不可用时，使用不重试的版本
    def _post_with_retry(url: str, *, headers: Dict[str, str], json: Dict[str, Any], timeout: int) -> requests.Response:
        """POST without retry (tenacity not available)."""
        return requests.post(url, headers=headers, json=json, timeout=timeout)

    def _get_with_retry(
        url: str, *, headers: Dict[str, str], params: Dict[str, Any], timeout: int
    ) -> requests.Response:
        """GET without retry (tenacity not available)."""
        return requests.get(url, headers=headers, params=params, timeout=timeout)
...
def fetch_url_content(url: str, timeout: int = 5) -> str:
    ...
    if not _newspaper_available:
        logger.debug(f"newspaper3k not installed, skipping content fetch for {url}")
        return ""
